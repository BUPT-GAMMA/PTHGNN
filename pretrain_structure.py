...
for epoch in np.arange(args.n_epoch) + 1:
    ...
    for batch in np.arange(repeat_num) + 1:
        ...
        for data, rem_edge_list, ori_edge_list, attr, (start_idx, end_idx) in train_data:
            ...
            # else:
            #     loss = loss_link
            loss = pthgnn.structure_loss(node_emb, rem_edge_list, ori_edge_list, node_dict, target_type, use_queue = True, update_queue=True, relation_level=args.use_relation)
        ...
        with torch.no_grad():
            ...
            node_emb = pthgnn.gnn(node_feature.to(device), node_type.to(device), edge_time.to(device), \
                                       edge_index.to(device), edge_type.to(device))
            loss_link = pthgnn.structure_loss(node_emb, rem_edge_list, ori_edge_list, node_dict, target_type, use_queue = True, update_queue=True, relation_level=args.use_relation)
