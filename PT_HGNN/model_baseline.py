    ...
    def link_loss(self, node_emb, rem_edge_list, ori_edge_list, node_dict, target_type, use_queue = True, update_queue = False, relation_level = False):
        ...
        for source_type in rem_edge_list:
            ...
            for relation_type in rem_edge_list[source_type]:
                ...
                target_ids = target_ids + node_dict[target_type][0]
                # print("target_ids like : ", target_ids)
                target_emb = node_emb[torch.LongTensor(target_ids)]

                # source_ids = torch.LongTensor(np.concatenate((positive_source_ids, negative_source_ids), axis=-1) + node_dict[source_type][0])
                source_ids = np.concatenate((positive_source_ids, negative_source_ids), axis=-1) + node_dict[source_type][0]
                positive_ids = torch.LongTensor(source_ids[:, 0].reshape(-1, 1))
                negative_ids = torch.LongTensor(source_ids[:, 1:])
                
                pos_emb = node_emb[positive_ids]
                neg_emb = node_emb[negative_ids]

                positive_score = torch.einsum('bid,bjd->bj', [target_emb, pos_emb])
                negative_score = torch.einsum('bid,bjd->bj', [target_emb, neg_emb])

                loss = self.bce_loss(positive_score, torch.ones_like(positive_score)) + self.bce_loss(negative_score, torch.zeros_like(negative_score))
                
                losses += loss.mean()
                # print("each relation : loss mean", loss.mean())
                # print("target shape : ", target_emb.shape)
                # print("positive shape : ", pos_emb.shape)
                # print('negative shape : ', neg_emb.shape)
                # print('positive score : ', pos_score.shape)
                # print('negative score : ', neg_score.shape)


                # pos_score = torch.sum(target_emb * )

                # loss = self.bce_loss()
            
                rep_size = sn + 1
                emb = node_emb[torch.LongTensor(source_ids)]
                source_emb = emb.reshape(source_ids.shape[0] * rep_size, -1)

                # print("emb shape : ", emb.shape)
                # print("source shape : ", source_emb.shape)
                target_ids = target_ids.repeat(rep_size, 1)
                target_emb = node_emb[target_ids.reshape(-1)]
                # print("target shape : ", target_emb.shape)
                res = self.cos(target_emb, source_emb)
...
class Matcher(nn.Module):
    ...
    #     # this linear layer represents the W \in R^(d*d)
